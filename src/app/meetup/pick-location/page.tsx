...
function PickLocationContent() {
  ...
  const [location, setLocation] = useState<PinData | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Click handler — place single marker
        map.on('click', (e) => {
          ...
          markerRef.current = new mb.Marker({ color: '#eab308' })
  ...
  const handleValidate = () => {
    ...
    const params = new URLSearchParams({
      ...
      alt: String(location.alt ?? ''),
    });
    router.push(`/meetup/new?${params.toString()}`);
  };
    ...
    <div className="relative w-screen h-screen overflow-y-auto flex flex-col">
      ...
      <div className="flex-1 relative" style={{ minHeight: 0 }}>
        ...
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-gray-900/85 backdrop-blur-sm text-white text-sm px-4 py-2 rounded-2xl shadow-lg text-center">
          <span>Touchez la carte pour placer le <span className="text-amber-400 font-semibold">rendez-vous</span></span>
        </div>
      ...
      <div className="flex-shrink-0 bg-white border-t border-gray-200 px-4 py-3 space-y-3 safe-area-bottom">
        {/* Pin info */}
        <div className={`text-xs px-3 py-2 rounded-xl ${location ? 'bg-amber-50 text-amber-700' : 'bg-gray-50 text-gray-400'}`}>
          📍 {location ? formatCoord(location) : 'Rendez-vous non placé'}
        </div>
      </div>
      ...
      <button
        ...
        style={{ bottom: '32px' }}
      >
        {location ? 'Valider' : 'Placez un point'}
      </button>
    </div>
  );
}
